...
#include <AFMotor.h>                //you must download and ins tall this library: https://drive.google.com/file/d/1zsMywqJjvzgMBoVZyrYly-2hXePFXFzw/view?usp=sharing
...
float xAxis, yAxis, beastModeAxis;
...
void loop() {
  while(bluetooth.available() > 0){                                   // This line checks for any new data in the buffer from the driverstation
    if ((bluetooth.read()) == 'z'){                                   // We use 'z' as a delimiter to ensure that the data doesn't desync
      ...
      beastModeAxis = (bluetooth.parseFloat());

      if (beastModeAxis <= -0.5) {
        beastMode = true;
      }
      if (beastModeAxis >= 0.5) {
        beastMode = false;
